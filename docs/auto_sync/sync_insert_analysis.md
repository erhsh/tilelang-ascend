...
用于记录和优化跨 pipeline 同步关系：

```
graph: {
  "PIPE_M":  {"PIPE_V", "PIPE_MTE1"},   // M 已同步到 V 和 MTE1
  "PIPE_V":  {"PIPE_MTE3"},              // V 已同步到 MTE3
}

传递闭包优化：
  已有 M→V 和 V→MTE3
  ⇒ 如果新需求是 M→MTE3，则跳过（已被间接满足）
```

#### BufferAccess（缓冲区访问记录）

```
BufferAccess {
  buffer_name:  "workspace_0"
  is_write:     true
  pipeline:     "PIPE_M"          // 该操作所属流水线
  operation:    "gemm_v0"         // 操作名称
  sync_graph:   { ... }           // 历史同步记录
  pipe_barriers: { "PipeBarrier_PIPE_M" }  // 已插入的 barrier
  physical_address: 0x1000        // 物理地址（用于判断内存重叠）
  is_sliced:    false             // 是否为切片操作
}
```

#### OperationConfig（操作配置表）

每种操作预定义了 buffer 访问模式和所属流水线：

| 操作 | Buffer 访问模式 | 所属 Pipeline |
|------|----------------|---------------|
| copy_gm_to_l1 | arg0=read, arg1=write | PIPE_MTE2 |
| copy_l1_to_l0a | arg0=read, arg1=write | PIPE_MTE1 |
| gemm_v0 / mma | arg0=read, arg1=read, arg2=write | PIPE_M |
| copy_ub_to_gm | arg0=read, arg1=write | PIPE_MTE3 |
| copy_l0c_to_gm | arg0=read, arg1=write | PIPE_FIX |
| AscendC::Add/Mul/Sub/Div... | arg0=write, arg1=read, arg2=read | PIPE_V |
| reduce_sum/max/min | arg0=write, arg1=read | PIPE_V |

#### EventMapping（事件映射表）

跨 pipeline 同步时，根据源/目标 pipeline 选择事件类型：

| 源 Pipeline → 目标 Pipeline | 事件类型 |
|----------------------------|---------|
| PIPE_M → PIPE_V | M_V |
| PIPE_V → PIPE_M | V_M |
| PIPE_MTE2 → PIPE_MTE1 | MTE2_MTE1 |
| PIPE_MTE1 → PIPE_M | MTE1_M |
| PIPE_M → PIPE_FIX | M_FIX |
| ... | （共 30+ 种映射） |

### 3.3 同步类型选择逻辑

```
判断数据依赖双方是否在同一 pipeline？
│
├─ 是 → 插入 PipeBarrier（同流水线屏障）
│        例：gemm_v0(PIPE_M) → gemm_v0(PIPE_M)
│        插入 ascend_auto_barrier("PIPE_M")
│
└─ 否 → 插入 EventPair（跨流水线事件对）
         例：gemm_v0(PIPE_M) → AscendC::Add(PIPE_V)
         插入 ascend_auto_set_flag("M_V", event_id)
              ascend_auto_wait_flag("M_V", event_id)
```
...
原始循环：
  for i in range(N):
    gemm(A, B, C)           // PIPE_M: write C
    add(C, D, E)            // PIPE_V: read C, write E

展开为两次迭代：
  iter1: gemm → sync? → add → sync?
  iter2: gemm → sync? → add → sync?

合并（取 iter1 和 iter2 同步的并集，去重）：
  for i in range(N):
    gemm(A, B, C)
    set_flag("M_V", eid)    // iter1: PIPE_M→PIPE_V
    wait_flag("M_V", eid)   // iter2: PIPE_M→PIPE_V（合并后去重）
    add(C, D, E)
```
...
处理后：
  ascend_auto_barrier("PIPE_ALL")     // ← 分支前：全量 barrier
  if (condition):
    op_A
    set_flag + wait_flag              // ← 分支内独立分析
    op_B
  else:
    op_C
  ascend_auto_barrier("PIPE_ALL")     // ← 分支后：全量 barrier
```

#### Buffer 切片操作

```
检测到 tvm_access_ptr 的 offset ≠ 0 时，标记为切片操作
→ 直接插入 PipeBarrier_ALL（粗粒度处理）
→ 原因：切片场景下精确的区间依赖分析复杂度高
```

#### ResourceScope（cube/vec 隔离）

```
遇到 resource_scope 属性时：
  - 保存当前 access_history
  - 清空 access_history（独立分析该 scope）
  - 处理完后恢复 access_history
  → cube 和 vec 的访问历史互不干扰
```

### 3.6 已支持功能

- 支持一般场景（即顺序场景、无分支、循环）下数据依赖间的自动插同步
- 通过 SyncGraph 传递闭包优化消除冗余的跨 pipeline EventPair 同步（已有 A→B + B→C 则不插 A→C）
...
按 workspace 名称分组：
  workspace_0:
    cube 侧: [write(pipe=MTE3), read(pipe=MTE2)]
    vec 侧:  [read(pipe=MTE2),   write(pipe=MTE3)]

匹配规则：
  1. 数量必须一致（cube_sps.size() == vec_sps.size()）
  2. 读写方向必须相反（cube.write ↔ vec.read）
  3. 按顺序逐对匹配，分配相同 flag_id

匹配失败处理：
  数量不一致 → LOG(FATAL) 直接终止编译
```

### 4.3 跨迭代同步（Cross Interval）

当 `cross_interval > 1` 时，同步不是每次迭代都执行，而是按间隔执行：

```
条件同步逻辑：
  Writer（set_flag）：
    条件：(stage_var % cross_interval == cross_interval - 1) || is_last_iteration
    含义：每隔 interval 次迭代或最后一次迭代时通知对方

  Reader（wait_flag）：
    条件：stage_var % cross_interval == 0
    含义：每隔 interval 次迭代时等待对方完成
```

### 4.4 目标循环深度选择（FindTargetLoopDepth）

```
目的：将同步语句附着到合适的循环层级，避免在过深或过浅的循环中插入

策略：
  1. 计算 cube/vec 各自父循环的总迭代次数
  2. 逐层对比，找到两侧迭代次数相等的最深层
  3. 将同步语句附着到该层循环（而非内部更深的循环）
  4. 跳过非共享的非常量循环（避免迭代次数无法比较）

示例：
  cube 侧: for i(N) → for j(4) → write_op     // 总 N*4 次
  vec 侧:  for i(N) → for k(4) → read_op       // 总 N*4 次
  → 匹配到 for i 层（两侧都是 N），同步附着到 for i
```

### 4.5 已支持功能

- 支持 CV 核间同步自动插入（cube/vec 通过 workspace 共享数据时的同步）
- 支持跨迭代条件同步（cross_interval 控制同步频率）
...
- 用户脚本中 workspace 读写次数不匹配，不支持核间同步自动插入（cube/vec 侧同步点数量不一致时直接报 FATAL 错误）
- 耦合在 CombineCV 中，需要单独拉出来（核间同步逻辑完全内嵌在 ascend_combinecv.cc 中，不是独立 Pass）
...
| 文件 | 职责 | 行数 |
|------|------|------|
| `src/transform/ascend_sync_insert.cc` | 核内同步：依赖分析 + 同步插入 | ~1600 |
| `src/transform/ascend_combinecv.cc` | CV分离 + 核间同步 | ~890 |
| `src/transform/cross_core_pipeline.cc` | 核间 Pipeline 拆分 | ~1220 |
| `src/transform/common/operation_config.h` | 操作配置表 + 事件映射表 | ~360 |
| `src/transform/ascend_memory_planning.cc` | 内存复用（为同步提供地址信息） | - |
| `tilelang/engine/phase.py` | Pass 流水线编排 | ~115 |
| `tilelang/transform/pass_config.py` | Pass 配置定义 | ~80 |

### 6.2 核心类关系

```
AscendSyncInsert（核内同步）
├── ForLoopUnroller       ── 展开 For 循环为两次迭代
├── LoopRebuilder          ── 合并迭代同步 + 重建 For 循环
│   ├── MergeStatementSequences  ── 合并 iter1/iter2 的同步语句
│   └── StmtFlattener     ── 展平语句序列
├── SyncGraph              ── 同步依赖图（传递闭包优化）
├── BufferAccess           ── 缓冲区访问记录
├── ExprAccessAnalyzer     ── 表达式级别的 buffer 访问分析
└── GetOperationConfig()   ── 操作配置表查询

CombineCV + AutoInsertCrossCoreSync（核间同步）
├── CVCombineEmitter       ── CV 分离（按操作类型过滤）
├── CrossCoreSyncCollector ── 同步点收集（GM 读写操作）
├── CrossCoreSyncInserter  ── 同步指令插入（set/wait cross flag）
└── FindTargetLoopDepth    ── 目标循环深度选择
```
